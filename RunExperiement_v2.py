...
class CrossValidation:
    ...
    def __init__(self, modelbuilder, datax, datay , logfile , cvparam = {'num_folds':10, 'shuffle': True, 'random_state':17}):
        ...
        self.folds = KFold(n_splits= cvparam['num_folds'],shuffle = cvparam['shuffle'])
        self.nfolds = cvparam["num_folds"]
        self.datax = datax
        self.datay = datay
    ...
    def evaluate(self, param, multithread = True):
        ...
        # call the same function with different data concurrently
                for result in pool.map(task, range(self.nfolds)):
                   score.append(result)
            print("Final CV score: "+ str(sum(score)/len(score)) + " for param" + str(param))
            self.log.write("Final CV score: "+ str(sum(score)/len(score)) + " for param" + str(param) + "\n")
            t2 = time.time()
...
class OAExperiment:
    ...
    def setrecord(self,optresults, testerr,cvtrainerr,evaltest):
        ...
        rtncol["Comment"] = self.experimentname +  self.comment 
...
spaceexp = {
            'max_depth': hp.uniformint('max_depth', 100,300),
            'learning_rate': hp.uniform('learning_rate', 0.01, 0.10),
            'num_leaves': hp.uniformint('num_leaves', 7, 15),
            'min_data_in_leaf':hp.uniformint('min_data_in_leaf',200,500),
            'nround': hp.uniformint('nround', 100,150),
            'lambda_l2': hp.uniform('lambda_l2 ', 0.0, 1.0),
        }

IntCols = ["max_depth","num_leaves","min_data_in_leaf","nround"]

DATAPATH = "/datassd/datastore/AuctionSignal/pytrain_ele69_20220301"
EXPERIEMNTNAME = "FullFeatureTest_v2_smallmodel"
DESCRIPTION= {"DataDescription":"intial batch of factors"}

SplitDate = datetime.datetime(2020,4,28)
TrainTestSplitFun = lambda x: splitfuc(x,"date",SplitDate)

MODEL = LightGBMModel
MAXEVAL = 400
CVParams = {'num_folds':5, 'shuffle': False}



exp = OAExperiment(DATAPATH, MODEL, TrainTestSplitFun ,SplitDate, spaceexp, MAXEVAL, CVParams, "SmallTrainSetLessTrees" ,DESCRIPTION ,experimentname = EXPERIEMNTNAME)

optrtn = exp.runfitting()

optrtn[0] = applyintmap(optrtn[0],IntCols)

bestmodel, testerr, cvtrainerr, evaltest, residualtrain, residualtest =  exp.getFullTests(optrtn[0])

exp.setrecord(optrtn, testerr,cvtrainerr,evaltest)

exp.dumpresult(optrtn, bestmodel,evaltest, residualtrain, residualtest)
